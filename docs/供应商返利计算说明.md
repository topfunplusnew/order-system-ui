...
> 精度工具：`packages/order-system/src/utils/rebateMath.js`（独立 math.js BigNumber 实例，至少 128 位有效精度，按输入跨度自动提高）
> 数值运算：以十进制文本直接构造 BigNumber，计算和比较不经过 `Number` / `number()`；结果以十进制字符串展示和提交。
...
计提金额 rebate = calculateRebateAmount(基数, unitPrice)
```

- 结果保留 **4 位小数**，使用 BigNumber 的四舍五入（ROUND_HALF_UP），不对中间面积/重箱值提前舍入。货物详情中的单行返利和合计保持原有 2 位小数。
...
flowchart TD
    ...
    C --> E["基数 × unitPrice"]
    D --> E
    E --> F["BigNumber 保留 4 位 → form.rebate 计提金额"]
    F --> G{可手工覆盖?}
...
1. **高精度**：面积、重箱、金额、流水汇总及大小比较均使用独立的 math.js BigNumber 实例，至少 128 位有效精度，按输入跨度自动提高，不修改其他页面的 math.js 配置。
2. **空值处理**：尺寸、片数缺失时按 BigNumber `0` 参与运算；非法文本、NaN、Infinity、已超出安全整数范围的 Number 会提示并阻止提交，不静默变成零。
3. **类型字段**：业务类型（返利 / 降价 / 售后质量赔偿）为 `rebateType`，与 **返利方式**（重箱/面积）不同，不参与上述乘积公式。
4. **传输边界**：金额、单价及计算出的面积/重箱以普通十进制字符串提交，避免 JSON 序列化前再转 Number 丢精度。后端应使用 BigDecimal 等十进制类型接收；超出 JS 安全范围的输入也须由接口以字符串返回，前端无法恢复接口 Number 已丢失的尾数。
5. **后端字段**：列表展示 `rebateMethod == 2` 为面积，否则为重箱；表单编辑期使用 `RebateType` 字符串，提交时转为 `1` / `2`。
...
| 计提金额 | BigNumber 计算 `Σ单项 × 单价` 后四舍五入到 4 位，Σ 按返利方式取面积或重箱 |
