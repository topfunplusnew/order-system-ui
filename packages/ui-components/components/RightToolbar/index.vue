...
<script>
// 默认的 ShowColumnsType 枚举
const ShowColumnsType = {
	TRANSFER: 'transfer',
	CHECKBOX: 'checkbox'
};
...
export default {
	...
	methods: {
		...
		async loadUserConfig() {
			if (!this.tableName || this.columns.length === 0 || !this.getUserConfig) return;
			try {
				const configKey = `column_config_${this.tableName}`;
				const response = await this.getUserConfig(configKey);
		...
		async saveUserConfig() {
			...
			try {
				...
				const configKey = `column_config_${this.tableName}`;
				await this.saveUserConfig(configKey, config);
			} catch (error) {
