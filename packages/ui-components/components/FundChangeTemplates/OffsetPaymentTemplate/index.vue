...
<script>
/**
 * OffsetPaymentTemplate - 冲抵款(cash_record)资金变动详情模板
 ...
 * targetDate:  String  日期范围过滤（utils/fundChange/dateScopedRows，日期字段 transactionTime）
 * summaryModuleLabel: String  默认'冲抵款'，用于记录列标题与汇总 label 前缀
 *
 * ── RENDER STRUCTURE（渲染结构）────────────────────────
 * 1. FundChangeExportButton   导出按钮（明细 + 差额汇总）
 * 2. el-table 主明细表（summaryOnly=false 时显示）
 *    固定列① 冲抵款  组内首行合并（默认3行），显示「冲抵款(N) + backupTime」
 *    固定列② 变更  修改前 / 修改后 / 差额（before-row / after-row / diff-row）
 *    数据列 19 列   = CASH_RECORD_COLUMNS 过滤 [amountDiff]
 *    合计行          仅差额行合计: amount（getTableSummary）
 * 3. el-table 底部汇总表（diff-summary-table，diffRows 存在时显示）
 *    buildBackendSummaryRows(summaryData, 'cash_record', prefix)
 ...
 *   status               | 状态           | CONST 固定'已冲抵'                        |
 *   tradeTime            | 交易时间       | info.transactionTime                       | ?? addtime，截取前10位
 *   amount               | 金额           | info.amount                               | 差额行回填差额
 *   payerName            | 支出方         | info.sourceCompanyName                     |
 *   receiverName         | 收入方         | info.targetCompanyName                     |
 *   payAccountName       | 支付户名       | info.payAccountName                        | ?? sourceAccountName ?? 支出方
 *   payAccountNo         | 支付账号       | info.payAccountNo                          | ?? sourceBankNo
 *   payerBank            | 支出方开户行   | info.payerBankName                         | ?? sourceBankName
 *   receiveAccountName   | 收款户名       | info.receiveAccountName                    | ?? targetAccountName ?? 收入方
 *   receiveAccountNo     | 收款账号       | info.receiveAccountNo                      | ?? targetBankNo
 *   receiverBank         | 收入方开户行   | info.receiverBankName                      | ?? targetBankName
 *   payerPayType         | 支出方支付类型 | info.sourcePayType                         | ?? sourcePaymentType
 *   payerCompanyType     | 支出方公司类型 | info.sourceCompanyType                     |
 *   receiverPayType      | 收入方支付类型 | info.targetPayType                         | ?? targetPaymentType
 *   receiverCompanyType  | 收入方公司类型 | info.targetCompanyType                     |
 *   offsetType           | 冲抵类型       | map{transfer:'内部转账',offset/cash_record:'冲抵'} | ?? info.type
 *   remark               | 备注           | info.remarks                              | ?? remark
 *   accountType          | 账户类型       | info.accountType                           |
 *   operatorName         | 操作人员姓名   | info.operatorName                          | ?? userName ?? createBy
 *
 * ── DIFF ROW（差额行 rowType='diff'，均为 修改后-修改前）────
 *   amountDiff 不直接展示（已从 columns 过滤），仅体现于差额行回填 amount 与合计行
 *   buildDiffFields: amount = calculateFieldDiff(changed.amount, original.amount) = 修改后 - 修改前
 *
 * ── BOTTOM SUMMARY（底部汇总表展示键）─────────────────
 *   默认输出键顺序 DEFAULT_OUTPUT_KEY_ORDER（有值才展示，label 前缀'冲抵款'）:
...
export default {
	...
	props: {
		...
		summaryModuleLabel: { type: String, default: '冲抵款' },
	...
	computed: {
		columns() {
			...
			return CASH_RECORD_COLUMNS.filter(c => !excludeProps.includes(c.prop)).map(c => (c.aggregator ? c : { ...c, showSummary: false }));
		},
		diffRows() {
