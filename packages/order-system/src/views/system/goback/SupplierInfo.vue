/Users/zhangming/Code/Projects/order-system-ui/packages/order-system/src/views/system/goback/SupplierInfo.vue<template>
	<div class="customer-info">
		...
		<el-table id="educe-table" :data="tableData" border style="width: 100%" v-loading="loading" size="mini" show-summary :summary-method="getSummaries">
			...
			<el-table-column label="供应商名称" width="150" prop="supplierName" show-overflow-tooltip>
				<template #default="scope">
					<el-tooltip effect="light" placement="top" enterable :open-delay="1000">
						<template slot="content">{{ scope.row.supplierName }}</template>
	...
	methods: {
		...
		handleExport() {
			this.$refs['form']?.validate(valid => {
				...
				this.download(`/statistics/export/supplierTrafficDetail?${query}`, {}, `供应商明细_${new Date().getTime()}.xlsx`);
			});
		},
		getSummaries(param) {
