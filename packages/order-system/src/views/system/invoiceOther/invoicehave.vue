...
<template>
	<div class="app-container">
		...
		<el-row :gutter="10" class="mb8">
			...
			<right-toolbar :showSearch.sync="showSearch" :columns="columns" @queryTable="getList" tableName="invoiceother-have-columns">
				<template #print>
					<el-col :span="1.5">
		...
		>
					...
					<el-tooltip effect="light" placement="top" enterable :open-delay="1000" :hide-after="0" popper-class="interactive-tooltip">
						<div slot="content" @click.stop>
							<div v-if="Array.isArray(scope.row.attachmentList)">
								<CheckFiles :attachmentList="scope.row.attachmentList" flag="paymentReceipts" @needToUpdate="value => handleUpdateFilePath(value, scope.row, getInvoiceOther, updateInvoiceOther)" />
							</div>
							<div v-else>
			...
			<el-table-column label="发票单" align="center" prop="invoiceAttachments">
				<template #default="scope">
					<el-tooltip effect="light" placement="top" enterable :open-delay="1000" :hide-after="0" popper-class="interactive-tooltip">
						<div slot="content" @click.stop>
							<div v-if="Array.isArray(scope.row.attachmentList)">
								<CheckFiles :attachmentList="scope.row.attachmentList" flag="invoiceAttachments" @needToUpdate="value => handleUpdateFilePath(value, scope.row, getInvoiceOther, updateInvoiceOther)" />
							</div>
							<div v-else>
								<el-tag type="danger">加载错误</el-tag>
							</div>
			...
			<el-table-column v-if="columns[13] && columns[13].visible" label="ID" align="center" prop="id" width="80" show-overflow-tooltip />
			<el-table-column v-if="columns[14] && columns[14].visible" label="新增时间" align="center" prop="addtime" width="170" show-overflow-tooltip />
				...
				<el-form ref="form" :model="form" :rules="rules" label-width="140px">
					...
					<el-col :span="12">
						...
						<el-form-item label="客户开票名称" prop="invoiceCompanyName">
							<!-- 2026-09-22 票点管理：仅手动填写客户开票名称时粘贴去除空白和其他标点，仅保留小括号和 -，系统选择的供应商/客户不限制 -->
	...
	data() {
		...
		return {
			...
			columns: [
				...
				{ key: 13, label: `ID`, visible: true },
				{ key: 14, label: `新增时间`, visible: true },
				{ key: 15, label: `最后修改时间`, visible: true },
				{ key: 16, label: `新增人`, visible: true },
				{ key: 17, label: `最后修改人`, visible: true }
			],
