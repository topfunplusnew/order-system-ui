<!-- 用户需求：新增质量赔偿统计表，订单指标来自订单，赔偿信息手动录入，剩余利润按公式计算。实际改动：复制销售奖励页面并固定为质量赔偿类型。 -->
<template>
	<div class="app-container">
		<div class="fixed-top-section">
			<el-form id="top-search-form-item" v-show="showSearch" ref="queryForm" :model="queryParams" size="mini" :inline="true" label-width="100px" class="form-container">
				<el-form-item label="质量问题简易描述" prop="qualityIssueDescription">
					<el-input v-model="queryParams.rewardReceiver" placeholder="请输入质量问题简易描述" clearable @keyup.enter.native="handleQuery" />
				</el-form-item>
		...
		<el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" :data="salesRewardList" show-summary :summary-method="getSummaries" border size="mini" :cell-style="() => ({ padding: '1px' })" @selection-change="handleSelectionChange">
			...
			<el-table-column v-if="columns[1].visible" label="质量问题简易描述" align="center" prop="qualityIssueDescription" min-width="120" show-overflow-tooltip />
			<el-table-column v-if="columns[2].visible" label="厂家赔偿我方金额" align="center" prop="manufacturerCompensationAmount" min-width="160" show-overflow-tooltip />
			<el-table-column v-if="columns[3].visible" label="我方赔偿客户金额" align="center" prop="customerCompensationAmount" width="120" show-overflow-tooltip>
				<template #default="scope">{{ formatAmount(scope.row.rewardAmount) }}</template>
			</el-table-column>
			<el-table-column v-if="columns[4].visible" label="厂家赔偿我方日期" align="center" prop="manufacturerCompensationDate" width="120" show-overflow-tooltip>
				<template #default="scope">{{ formatAmount(scope.row.paymentAmount) }}</template>
			</el-table-column>
			<el-table-column v-if="columns[5].visible" label="我方赔偿客户日期" align="center" prop="customerCompensationDate" width="120" show-overflow-tooltip>
				<template #default="scope">{{ scope.row.rewardDate ? parseTime(scope.row.rewardDate, '{y}-{m}-{d}') : '-' }}</template>
			</el-table-column>
			<el-table-column label="质量赔偿后剩余利润" prop="remainingProfit" width="130">
				<template #default="scope">{{ formatAmount(scope.row.remainingProfit !== undefined ? scope.row.remainingProfit : Number(scope.row.comprehensiveProfit || 0) + Number(scope.row.manufacturerCompensationAmount || 0) - Number(scope.row.customerCompensationAmount || 0)) }}</template>
			</el-table-column>
			...
			<el-table-column v-if="columns[10].visible" label="操作" align="center" class-name="small-padding fixed-width" width="340" fixed="right">
				<template #default="scope">
					...
					<el-button v-if="false" v-hasPermi="['system:salesReward:supplement']" size="mini" type="text" icon="el-icon-edit-outline" :disabled="isSupplementDisabled(scope.row)" @click="handleSupplement(scope.row)">补充信息</el-button>
		...
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :title="title" :visible.sync="open" width="640px" append-to-body>
			<el-form ref="form" :model="form" :rules="rules" label-width="120px">
				<el-form-item label="质量问题简易描述" prop="qualityIssueDescription">
					<el-input v-model="form.rewardReceiver" placeholder="请输入质量问题简易描述" />
				</el-form-item>
				<el-form-item label="厂家赔偿我方金额" prop="manufacturerCompensationAmount">
					<el-input v-model="form.rewardReason" placeholder="请输入厂家赔偿我方金额" />
				</el-form-item>
				<el-form-item label="我方赔偿客户金额" prop="customerCompensationAmount">
					<el-input v-model="form.rewardAmount" placeholder="请输入我方赔偿客户金额" @blur="formatAmountField('rewardAmount')" />
				</el-form-item>
				<el-form-item label="审核状态" prop="auditState">
		...
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight title="补充信息" :visible.sync="supplementOpen" width="500px" append-to-body>
			<el-form ref="supplementForm" :model="supplementForm" :rules="supplementRules" label-width="150px">
				<el-form-item label="厂家赔偿我方日期" prop="manufacturerCompensationDate">
					<el-input v-model="supplementForm.paymentAmount" placeholder="请输入厂家赔偿我方日期" @blur="formatSupplementAmountField('paymentAmount')" />
				</el-form-item>
				<el-form-item label="我方赔偿客户日期" prop="customerCompensationDate">
					<el-date-picker v-model="supplementForm.rewardDate" clearable type="date" value-format="yyyy-MM-dd" placeholder="请选择我方赔偿客户日期" style="width: 100%" />
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
...
export default {
	...
	data() {
		...
		return {
			...
			form: {},
			rules: {
				rewardReceiver: [{ required: true, message: '请输入质量问题简易描述', trigger: 'blur' }],
				rewardReason: [{ required: true, message: '请输入厂家赔偿我方金额', trigger: 'blur' }],
				rewardAmount: [{ validator: amountValidator('请输入我方赔偿客户金额', true), trigger: 'blur' }],
				auditState: [{ required: true, message: '请选择审核状态', trigger: 'change' }]
			},
			supplementRules: {
				paymentAmount: [{ validator: amountValidator('请输入厂家赔偿我方日期'), trigger: 'blur' }],
				rewardDate: [
					{
						validator: (rule, value, callback) => {
							if (!value && !this.supplementForm.paymentAmount) {
								callback(new Error('请至少填写支付金额或我方赔偿客户日期其中一个'));
								return;
							}
							callback();
						},
			...
			columns: [
				...
				{ key: 1, label: '质量问题简易描述', visible: true },
				{ key: 2, label: '厂家赔偿我方金额', visible: true },
				{ key: 3, label: '我方赔偿客户金额', visible: true },
				{ key: 4, label: '厂家赔偿我方日期', visible: true },
				{ key: 5, label: '我方赔偿客户日期', visible: true },
	...
	methods: {
		...
		submitSupplementForm() {
			this.$refs['supplementForm'].validate(valid => {
				...
				if (!this.supplementForm.paymentAmount && !this.supplementForm.rewardDate) {
					this.$message.error('请至少填写支付金额或我方赔偿客户日期其中一个');
					return;
				}
		...
		handleExport() {
			...
			this.download('system/salesReward/export', exportParams, `质量赔偿_${new Date().getTime()}.xlsx`);
		}
	}
};
</script>
