...
const MANUAL_WIDTH_LOCK_KEY = '__manualWidthLocked';
...
function getDescendantContentWidth(content) {
	...
	const targets = new Set(
		Array.from(content.querySelectorAll('*')).filter(element => {
			if (typeof element.matches === 'function' && element.matches(CONTENT_WIDTH_SELECTOR)) {
				return true;
			}
...
function syncColumnWidthToDom(tableVm, column, width) {
	if (!tableVm || !tableVm.$el || !column || !column.id) {
		return;
	}

	tableVm.$el.querySelectorAll(`col[name="${column.id}"]`).forEach(col => {
		col.setAttribute('width', String(width));
	});
}

export function lockManualColumnWidth(tableVm, column, width) {
	if (!column || !column.id) {
		return;
	}
	...
	column[AUTO_WIDTH_MANAGED_KEY] = false;
	column[MANUAL_WIDTH_LOCK_KEY] = true;
	column.width = normalizedWidth;
	column.realWidth = normalizedWidth;

	syncColumnWidthToDom(tableVm, column, normalizedWidth);
}
...
export function isAutoWidthExcludedColumn(column, cells = []) {
	...
	if (column[MANUAL_WIDTH_LOCK_KEY]) {
		return true;
	}
...
export function applyAutoWidthToTable(tableVm, options = {}) {
	...
	columns.forEach(column => {
		...
		column[MANUAL_WIDTH_LOCK_KEY] = false;
...
export function createTableAutoWidthMixin(defaultOptions = {}) {
	...
	return {
		mounted() {
			...
			this.__bindTableAutoWidthHeaderDragEnd();
		...
		methods: {
			...
			__bindTableAutoWidthHeaderDragEnd() {
				if (this.__tableAutoWidthDragEndBound || typeof this.$on !== 'function') {
					return;
				}

				this.__tableAutoWidthHeaderDragEndHandler = (newWidth, oldWidth, column) => {
					lockManualColumnWidth(this, column, newWidth);
				};
				this.$on('header-dragend', this.__tableAutoWidthHeaderDragEndHandler);
				this.__tableAutoWidthDragEndBound = true;
			},
			__cleanupTableAutoWidth() {
				...
				if (this.__tableAutoWidthHeaderDragEndHandler && typeof this.$off === 'function') {
					this.$off('header-dragend', this.__tableAutoWidthHeaderDragEndHandler);
				}

				this.__tableAutoWidthResizeHandler = null;
				this.__tableAutoWidthHeaderDragEndHandler = null;
				this.__tableAutoWidthResizeBound = false;
				this.__tableAutoWidthDragEndBound = false;
			}
		}
	};
}
