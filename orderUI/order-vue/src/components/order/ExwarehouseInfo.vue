    ...
    <div style="height: 800px;">
        <lay-table
            ...
            <template v-slot:toolbar>
                <lay-button size="sm" type="primary" @click="handleAdd">新增车辆信息</lay-button>
                <lay-button size="sm" @click="remove">删除</lay-button>
                <lay-input :allow-clear="true" v-model="valueInputNumber"
                           style="width: 150px;height: 33px;margin-left: 10px" placeholder="请输入车牌"></lay-input>
                <lay-input :allow-clear="true" v-model="valueInputName"
                           style="width: 150px;height: 33px;margin-left: 10px" placeholder="请输入户名"></lay-input>
                <lay-button size="sm">搜索</lay-button>
            </template>
...
//弹窗
const handleAction = (): AxiosPromise => {
    return new Promise(() => {
...
const changeVisible = (row) => {
    modelPop.value = row;
    visiblePop.value = !visiblePop.value;
}
const {visiblePop, modelPop, actionPop} = usePopForm(handleAction)
const {visibleAdd, modelAdd, actionAdd} = useAddForm(handleAction)
</script>
