<template>
	<div class="login">
		...
		<div class="el-login-footer">
			<span>温馨提示：推荐使用IE9以上浏览器以及360极速模式。</span>
		</div>
	</div>
</template>
		...
		handleLogin() {
			this.$refs.loginForm.validate(valid => {
				if (valid) {
					this.loading = true;
					// 始终设置较长的过期时间（单位：天）
					Cookies.set('username', this.loginForm.username, { expires: 9999 });
					Cookies.set('password', encrypt(this.loginForm.password), { expires: 9999 });
					this.$store
