<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8 fixed-toolbar" :style="{ top: getToolbarTop() }">
      <el-col>
...
export default {
  ...
  methods: {
    ...
    // 计算工具栏的 top 值
    getToolbarTop() {
      // 如果搜索表单显示，工具栏应该在搜索表单下方
      return this.showSearch ? '60px' : '0px';
    },
    // 计算表格高度
    getTableHeight() {
      // 根据窗口高度动态计算表格高度，减去其他组件占用的空间
      const searchFormHeight = this.showSearch ? 60 : 0; // 搜索表单高度
      const toolbarHeight = 50; // 工具栏高度
      const otherSpace = 40; // 其他间距
      return window.innerHeight - searchFormHeight - toolbarHeight - otherSpace;
    },
    // 获取当前日期的方法
    getCurrentDateTime() {
      const now = new Date();
    ...
    getList() {
      ...
      listBankAccountSelf(apiParams).then(response => {
        this.bankAccountList = response.rows || response.data;
