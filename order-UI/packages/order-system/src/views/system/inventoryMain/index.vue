				...
				<el-form-item label="仓库名称" prop="storeHouseName">
					...
					<el-col :span="8">
						<SearchOption
							...
							@commitBack="
								value => handleCommitBackInventory(value)
							"
							...
							<el-row>
								...
								<el-col :span="6">
									<SearchOption
										...
										@commitBack="
											value =>
												handleCommitBackSupplier(
													scope,
													value
												)
										"
										@update:queryName="
											handleUpdateQuerySupplier
										"
										@click="
											setCurrentType(
												scope.row,
	...
	methods: {
			...
			function calculateProfitNoTax() {
				if (
					...
					return fix(
						scope.row.payments -
							scope.row.paymentFactory -
							(scope.row.landFreight + scope.row.seaFreight) *
								1.075 -
							(((scope.row.height *
								scope.row.length *
								scope.row.width *
				...
				if (valid) {
					...
					this.form.allLandFreight = this.isLand
						? this.inventoryDetailList.reduce(
								(prev, curr) =>
									Number(prev) + Number(curr.landFreight),
								0
						  )
						: 0;
					// 计算海运费
					this.form.allSeaFreight = this.isSea
						? this.inventoryDetailList.reduce(
								(prev, curr) =>
									Number(prev) + Number(curr.seaFreight),
								0
						  )
						: 0;
		...
		handleDeleteInventoryDetail() {
			if (this.checkedInventoryDetail.length == 0) {
				...
				this.inventoryDetailList = inventoryDetailList.filter(function (
					item
				) {
