<template>
  <div class="app-container">
    <el-form id="top-search-form-item" :model="queryParams" ref="queryForm" size="mini" :inline="true"
             v-show="showSearch && isConfigLoaded" label-width="150">
      <el-form-item v-if="shouldShowField('storeHouseName')" label="仓库名称" prop="storeHouseName">
        <el-input v-model="queryParams.storeHouseName" class="input-standard" placeholder="请输入仓库名称" clearable
      ...
      <el-form-item v-if="shouldShowField('userName')" label="录入人员" prop="userName">
        <el-input v-model="queryParams.userName" class="input-standard" placeholder="录入人员" clearable
                  @keyup.enter.native="handleQuery" @input="val => (queryParams.UserName = val)"/>
      </el-form-item>
      <el-form-item v-if="shouldShowField('supplier')" label="供应商" prop="supplier">
        <el-input v-model="queryParams.params.detail_supplier" class="input-standard" placeholder="请输入供应商"
                  clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item v-if="shouldShowField('goodsCompany')" label="货物来源" prop="goodsCompany">
        <el-input v-model="queryParams.goodsCompany" class="input-standard" placeholder="请输入货物来源公司" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item v-if="shouldShowField('landCarNo')" label="陆运车牌" prop="landCarNo">
        <el-input v-model="queryParams.landCarNo" class="input-standard" placeholder="请输入陆运车牌" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item v-if="shouldShowField('seaDriverName')" label="海运公司" prop="seaDriverName">
        <el-input v-model="queryParams.seaDriverName" class="input-standard" placeholder="请输入海运公司" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <right-toolbar :showSearch.sync="showSearch" :columns="columns" @queryTable="getList">
          ...
          <el-row :gutter="10">
            <el-col :span="1.5">
              <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                         v-hasPermi="['system:inventoryMain:add']">新增
              </el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button type="primary" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
                         v-hasPermi="['system:inventoryMain:edit']">修改
              </el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple"
                         @click="handleDelete" v-hasPermi="['system:inventoryMain:remove']">删除
              </el-button>
            </el-col>
            <!-- <el-col :span="1.5">
        ...
        <template #export>
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:inventoryMain:export']" plain icon="el-icon-folder-opened" size="mini"
                       @click="handleExport">导出库存目录
            </el-button>
            <el-button v-hasPermi="['system:inventoryMain:export']" plain icon="el-icon-folder-opened" size="mini"
                       @click="handleExportNoPage">导出全部库存
            </el-button>
          </el-col>
        </template>
      </right-toolbar>
    ...
    <div>
      <DragDiv :initial-left-width="300" :min-left-width="200" :min-right-width="800" :divider-width="6"
               @drag-start="handleDragStart" @dragging="handleDragging" @drag-end="handleDragEnd">
        <!-- 左侧：仓库树 -->
        <template #left>
          <div class="tree-container"
               style="height: 100%; padding: 10px; background: #fafafa; border: 1px solid #e6e6e6; display: flex; flex-direction: column">
        ...
        <template #right>
          <div style="height: 100%; display: flex; flex-direction: column">
            <u-table border id="printBox" size="mini" v-loading="loading" :data="inventoryMainList"
              ...
              <CustomTableColumn v-if="columns[1].visible" label="仓库名称" align="center" prop="storeHouseName"
                                 width="150"/>
              <CustomTableColumn v-if="columns[2].visible" label="变动日期(入库)" align="center" prop="storeDate"
                                 width="150">
              ...
              <CustomTableColumn v-if="columns[4].visible" label="货物来源公司" align="center" prop="goodsCompany"
                                 width="180"/>
              <!-- 修改审核状态列 -->
              <CustomTableColumn v-if="columns[5].visible" label="审核状态" align="center" prop="checkState"
                                 width="150">
                <template #default="scope">
              ...
              <CustomTableColumn v-if="columns[6].visible" label="陆运车牌" align="center" prop="landCarNo"
                                 width="120"/>
              <CustomTableColumn v-if="columns[7].visible" label="陆运司机电话" align="center" prop="landDriverTel"
                                 width="150"/>
              <CustomTableColumn v-if="columns[8].visible" label="陆地司机姓名" align="center" prop="landDriverName"
                                 width="120"/>
              <CustomTableColumn v-if="columns[9].visible" label="陆运银行卡号" align="center" prop="landBankNo"
                                 width="120"/>
              <CustomTableColumn v-if="columns[10].visible" label="陆运银行户名" align="center" prop="landBankName"
                                 width="120"/>
              <CustomTableColumn v-if="columns[11].visible" label="柜号" align="center" prop="seaCarNo" width="120"/>
              <CustomTableColumn v-if="columns[12].visible" label="海运司机电话" align="center" prop="seaDriverTel"
                                 width="150"/>
              <CustomTableColumn v-if="columns[13].visible" label="海运公司" align="center" prop="seaDriverName"
                                 width="120"/>
              <CustomTableColumn v-if="columns[14].visible" label="海运银行卡号" align="center" prop="seaBankNo"
                                 width="120"/>
              <CustomTableColumn v-if="columns[15].visible" label="海运银行户名" align="center" prop="seaBankName"
                                 width="120"/>
              <CustomTableColumn v-if="columns[16].visible" label="子项陆运费之和" align="center" prop="allLandFreight"
                                 width="150"/>
              <CustomTableColumn v-if="columns[17].visible" label="子项海运费之和" align="center" prop="allSeaFreight"/>
              <CustomTableColumn v-if="columns[18].visible" label="录入人员" align="center" prop="userName"
                                 width="120"/>
              <CustomTableColumn v-if="columns[19].visible" label="附件" align="center" prop="path" width="150"
                                 fixed="right">
                <template #default="scope">
              ...
              <CustomTableColumn v-if="columns[20].visible" label="收到条附件" align="center" prop="receiveProof"
              ...
              <CustomTableColumn v-if="columns[21].visible" label="操作" align="center" width="300" fixed="right">
                <template slot-scope="scope">
                  <el-button size="mini" type="text" icon="el-icon-edit" @click="handleCheckInventory(scope.row)">查看
                  </el-button>
                  <el-tooltip effect="light" v-if="isInventoryDisabledModify(scope.row)" placement="top">
                    <template #content>
                      <div>该货物为二次入库货物，请在存货二次加工管理处修改</div>
                    </template>
                    <el-button :disabled="true" size="mini" type="text" icon="el-icon-edit"
                               v-hasPermi="['system:inventoryMain:edit']">修改
                    </el-button>
                  </el-tooltip>
                  <el-button
                  ...
                  <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                             v-hasPermi="['system:inventoryMain:remove']">删除
                  </el-button>
                </template>
              </CustomTableColumn>
            </u-table>
    ...
    <!-- 添加或修改库存库存主表对话框 -->
    <el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :title="title" :visible.sync="open"
               width="100%" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" :inline="true">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <!-- 删除按钮：仅在选中项时可用 -->
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="handleDeleteInventoryDetail"
                       :disabled="checkedInventoryDetail.length === 0">删除
            </el-button>
          </el-col>
          <!-- 编辑子项按钮：有子项且没有行处于编辑状态时可用 -->
          <el-col :span="1.5">
            <el-button size="mini" type="warning" @click="toggleEditDetails(true)"
                       :disabled="!hasInventoryDetails || hasEditingRows">编辑子项
            </el-button>
          </el-col>
          <!-- 全部保存按钮：有行处于编辑状态时可用 -->
          <el-col :span="1.5">
            <el-button size="mini" type="success" @click="toggleEditDetails(false)" :disabled="!hasEditingRows">
              全部保存
            </el-button>
          </el-col>
        </el-row>
            ...
            <template slot-scope="scope">
              <div>
                <el-button v-if="!scope.row.isEditing" :disabled="!isEditingDetails" size="mini" type="warning"
                           icon="el-icon-edit" @click="handleRowEdit(scope.row)">编辑
                </el-button>
                <el-button v-else size="mini" type="success" icon="el-icon-check" @click="handleRowSave(scope.row)">
                  保存
                </el-button>
                <el-button :disabled="scope.row.isEditing" size="mini" type="danger" icon="el-icon-document-copy"
                           @click="handleCopyRow(scope.row)">复制
                </el-button>
              </div>
            </template>
          </el-table-column>
