...
<template>
	<div class="app-container">
		<el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="mini" :inline="true"
			...
			<el-form-item label="加油卡卡号" prop="oilCardNo">
				<el-input v-model="queryParams.oilCardNo" placeholder="请输入加油卡卡号" clearable
					@keyup.enter.native="handleQuery" @input="handleInputTrim($event, 'queryParams', 'oilCardNo')" />
			</el-form-item>
			<el-form-item>
		...
		<el-row :gutter="10" class="mb8">
			...
			<el-col :span="1.5">
				<el-button v-hasPermi="['system:oilcard:add']" type="primary" size="mini"
					@click="handleAdd">新增加油卡信息</el-button>
			</el-col>
			<el-col :span="1.5">
			...
			<right-toolbar :showSearch.sync="showSearch" :columns="columns" @queryTable="getList">
				...
				<template #export>
					<el-col :span="1.5">
						<el-button v-hasPermi="['system:oilcard:export']" plain icon="el-icon-folder-opened" size="mini"
		...
		<el-row>
			<el-tabs v-model="activeName" @tab-click="handleClick">
				<el-tab-pane lazy label="加油卡主卡管理" name="first">
					<el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" size="mini" border
						:data="mainOilCardList" @selection-change="handleSelectionChange">
						...
						<el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right">
							<template slot-scope="scope">
								<el-button v-hasPermi="['system:oilcard:query']" size="mini" type="text"
									@click="handleCheck(scope.row)">查看明细</el-button>
								<el-button v-hasPermi="['system:oilcard:edit']" size="mini" type="primary"
									@click="handleUpdate(scope.row)">编辑</el-button>
								<el-button v-hasPermi="['system:oilcard:remove']" size="mini" type="danger"
									@click="handleDelete(scope.row)">删除</el-button>
							</template>
						</el-table-column>
					</el-table>
					<pagination v-show="MainTotal > 0" :total="MainTotal" :page.sync="queryParams.pageNum"
						:limit.sync="queryParams.pageSize" @pagination="getList()" />
					<br />
				</el-tab-pane>
				<el-tab-pane lazy label="加油卡副卡管理" name="second">
					<el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" border size="mini"
						:data="subCardList" @selection-change="handleSelectionChange">
						<el-table-column v-if="columns[0].visible" label="加油卡卡号" align="center" prop="oilCardNo" />
					...
					<pagination v-show="SubTotal > 0" :total="SubTotal" :page.sync="queryParams.pageNum"
						:limit.sync="queryParams.pageSize" @pagination="getList" />
					<br />
				</el-tab-pane>
			</el-tabs>
		...
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :close-on-click-modal="false"
			...
			<el-form ref="form" :model="form" :rules="rules" label-width="120px" @keyup.enter.native="submitForm"
				size="mini" @submit.native.prevent="submitForm">
				<el-form-item label="加油卡卡号" prop="oilCardNo">
					<el-input v-model="form.oilCardNo" placeholder="请输入加油卡卡号"
						@input="handleInputTrim($event, 'form', 'oilCardNo')" />
				</el-form-item>
				...
				<el-form-item label="备注" prop="comments">
					<el-input v-model="form.comments" placeholder="请输入使用备注"
						@input="handleInputTrim($event, 'form', 'comments')" />
				</el-form-item>
			</el-form>
		...
		<!--    加油卡充值弹窗-->
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :close-on-click-modal="false"
			:show-close="false" title="加油卡充值" :visible.sync="moneyDialogVisible" width="500px" append-to-body>
			<el-form :model="moneyInfo" :rules="moneyRules" label-width="120px" @keyup.enter.native="submitMoney"
				@submit.native.prevent="submitMoney" ref="moneyFormRef" size="mini">
				<el-form-item label="加油卡卡号" prop="oilCardNo">
					<el-row>
						<el-col :span="10">
							<el-input v-model="moneyInfo.oilCardNo" placeholder="请输入加油卡卡号"
								@input="handleInputTrim($event, 'moneyInfo', 'oilCardNo')" />
						</el-col>
						<el-col :span="3">
							<SearchOption :get-data="listOilCard" title="选择加油卡" icon="el-icon-search"
								:limit-info="{ oilType: '主卡' }" @commitBack="handleCommitBack"
								@update:queryName="handleUpdateQueryName" :query-name="queryCard" query-info="oilCardNo"
								query-label="加油卡卡号">
				...
				<el-form-item label="请输入充值金额" prop="rechargeMoney">
					<el-input v-model="moneyInfo.rechargeMoney" placeholder="请输入充值金额"
						@input="handleInputTrim($event, 'moneyInfo', 'rechargeMoney')" />
				</el-form-item>
				...
				<el-form-item v-if="moneyInfo.rechargeType === '银行卡'" label="银行开户名" prop="acountsName">
					<el-col :span="10">
						<el-input v-model="moneyInfo.acountsName" placeholder="请输入银行开户名"
							@input="handleInputTrim($event, 'moneyInfo', 'acountsName')" />
					</el-col>
					<el-col :span="4">
				...
				<el-form-item v-if="moneyInfo.rechargeType === '银行卡'" label="银行账号" prop="bankNo">
					<el-input v-model="moneyInfo.bankNo" placeholder="请输入银行账号"
						@input="handleInputTrim($event, 'moneyInfo', 'bankNo')" />
				</el-form-item>
				<el-form-item label="附件" prop="bankName">
					<UploadFilesButton ref="attachmentUploader" :table-name="'oilcard'" :record-id="moneyInfo.id"
						:attachment-type="'附件'" :initial-attachments="(form.params && form.params.attachments) || []"
		...
		<!--    加油卡操作功能-->
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :close-on-click-modal="false"
			:show-close="false" title="加油卡操作" :visible.sync="optionVisible" width="500px" append-to-body>
			<el-form ref="oilCardForm" :model="oilCardOption" :rules="optionRules" label-width="120px" size="mini"
				@keyup.enter.native="submitOilCardOption" @submit.native.prevent="submitOilCardOption">
				...
				<el-form-item label="主加油卡卡号" prop="oilMainCardNo"
					v-if="oilCardOption.type === OilCardOptionType.MAIN_TO_SUB">
					<el-row>
						<el-col :span="10">
				...
				<el-form-item label="副加油卡卡号" prop="oilSecondCardNo">
					<el-row>
						...
						<el-col :span="4">
							<SearchOption :get-data="listOilCard" query-info="oilCardNo" :query-name="queryOilCardOther"
								query-label="油卡账号查询" :limit-info="{ oilType: '副卡' }"
				...
				<el-form-item :label="oilCardOption.type === OilCardOptionType.MAIN_TO_SUB ? `分配金额` : `圈存金额`"
					prop="rechargeMoney">
					<el-input v-model="oilCardOption.rechargeMoney"
				...
				<el-form-item label="备注" prop="comments">
					<el-input v-model="oilCardOption.comments" placeholder="请输入备注"
						@input="handleInputTrim($event, 'oilCardOption', 'comments')" />
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
