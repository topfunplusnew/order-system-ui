	...
	<div class="app-container">
		<el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="mini" :inline="true" label-width="68px">
			...
			<el-form-item label="用车时间" prop="startTime">
				<el-date-picker v-model="queryParams.startTime" type="datetime" placeholder="选择用车时间"
					value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
			</el-form-item>
			<el-form-item label="还车时间" prop="endTime">
				<el-date-picker v-model="queryParams.endTime" type="datetime" placeholder="选择还车时间"
					value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
			</el-form-item>
	
			<el-form-item label="派车人" prop="dispatchPerson">
				<el-input v-model="queryParams.dispatchPerson" placeholder="请输入派车人" clearable
		...
		<el-row :gutter="10" class="mb8">
			...
			<right-toolbar :showSearch.sync="showSearch" :columns="columns" @queryTable="getList">
				...
				<template #export>
					<el-col :span="1.5">
						<el-button v-hasPermi="['system:carapply:export']" plain icon="el-icon-folder-opened" size="mini"
		...
		<el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" border :data="carApplyList" size="mini"
			:cell-style="
								() => {
			...
			<el-table-column v-if="columns[8].visible" label="用车事由" align="center" prop="applyPurpose"
				show-overflow-tooltip />
			<el-table-column v-if="columns[9].visible" label="出车前里程" align="center" prop="startMile"
				show-overflow-tooltip />
			<el-table-column v-if="columns[10].visible" label="出车前车况" align="center" prop="startCarState"
				show-overflow-tooltip />
			<el-table-column v-if="columns[11].visible" label="回来后里程" align="center" prop="endMile" show-overflow-tooltip />
			<el-table-column v-if="columns[12].visible" label="回来后车况" align="center" prop="endCarState"
				show-overflow-tooltip />
			<el-table-column v-if="columns[13].visible" label="用车里程数" align="center" prop="miles" show-overflow-tooltip />
			<el-table-column v-if="columns[14].visible" label="回程停靠位置" align="center" prop="backStopPlace"
				show-overflow-tooltip />
			<el-table-column v-if="columns[15].visible" label="行程中违法次数" align="center" prop="violationsCount"
				show-overflow-tooltip />
			<el-table-column v-if="columns[16].visible" label="违章罚款金额" align="center" prop="fine" show-overflow-tooltip />
			<el-table-column v-if="columns[17].visible" label="行程中是否维修/保养" align="center" prop="isMaintenance"
				show-overflow-tooltip />
			<el-table-column v-if="columns[18].visible" label="保养金额" align="center" prop="maintenanceMoney"
				show-overflow-tooltip />
			<el-table-column v-if="columns[19].visible" label="维修金额" align="center" prop="repairMoney"
				show-overflow-tooltip />
			<el-table-column v-if="columns[20].visible" label="行程中使用加油卡加油次数" align="center" prop="refuelingFrequency"
				show-overflow-tooltip />
			<el-table-column v-if="columns[21].visible" label="加油金额" align="center" prop="refuelingMoney"
				show-overflow-tooltip />
			<el-table-column v-if="columns[22].visible" label="现金加油次数" align="center" prop="cashRefuelingFrequency"
				show-overflow-tooltip />
			<el-table-column v-if="columns[23].visible" label="加油卡余额" align="center" prop="oilCardBalance"
				show-overflow-tooltip />
			<el-table-column v-if="columns[24].visible" label="加油小票是否交回公司" align="center" prop="isTicketReturned"
				show-overflow-tooltip />
			<el-table-column v-if="columns[25].visible" label="现金加油金额" align="center" prop="cashRefueling"
				show-overflow-tooltip />
			<el-table-column v-if="columns[26].visible" label="派车人" align="center" prop="dispatchPerson"
				show-overflow-tooltip />
			...
			<el-table-column v-if="columns[28].visible" label="附件" align="center" prop="attachmentList">
				<template slot-scope="scope">
					<div v-if="Array.isArray(scope.row.attachmentList)">
						<CheckFiles :attachmentList="scope.row.attachmentList" :flag="'attachments'"
							@needToUpdate="value => handleUpdateFilePath(value, scope.row, getCarApply, updateCarApply)" />
					</div>
					<div v-else>
			...
			<el-table-column v-if="columns[29].visible" label="审核状态" align="center" prop="auditState" width="120px">
				<template slot-scope="scope">
					<el-tag
						:type="scope.row.auditState === '审核通过' ? 'success' : scope.row.auditState === '审核不通过' ? 'danger' : 'info'">
			...
			<el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="180px">
				<template slot-scope="scope">
					<el-button v-hasPermi="['system:carapply:edit']" size="mini" type="primary"
						@click="handleUpdate(scope.row)">修改</el-button>
					<el-button v-hasPermi="['system:carapply:remove']" size="mini" type="danger"
						@click="handleDelete(scope.row)">删除</el-button>
					<el-button v-hasPermi="['system:carapply:audit']" size="mini" type="warning"
						@click="handleAudit(scope.row)" v-if="scope.row.auditState === null">派车审核</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :modal="false" v-dialogDrag v-dialogDragWidth v-dialogDragHeight :close-on-click-modal="false"
			:show-close="false" :title="title" :visible.sync="open" width="1500px" append-to-body>
			<div>
				<el-form ref="form" :model="form" :rules="rules" label-width="160px">
					<el-row>
						<el-col :span="8">
							<el-form-item label="申请时间" prop="applyDate">
								<el-date-picker v-model="form.applyDate" type="datetime" placeholder="选择申请时间"
									value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
							</el-form-item>
							...
							<el-form-item label="车牌" prop="carNo">
								<el-row>
									...
									<el-col :span="4">
										<SearchOption :limit-info="{}" :get-data="listVehicles" query-label="车牌搜索"
											:query-name="queryCars" query-info="licensePlate"
							...
							<el-form-item label="用车时间" prop="startTime">
								<el-date-picker v-model="form.startTime" type="datetime" placeholder="选择用车时间"
									value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
							</el-form-item>
							<el-form-item label="还车时间" prop="endTime">
								<el-date-picker v-model="form.endTime" type="datetime" placeholder="选择还车时间"
									value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
							</el-form-item>
