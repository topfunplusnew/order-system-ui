  ...
  <div>
        ...
        <template #export>
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:goodsorder:export']" plain icon="el-icon-folder-opened" size="mini"
                       @click="handleExport">导出订单目录
            </el-button>
            <el-button v-hasPermi="['system:goodsorder:export']" plain icon="el-icon-folder-opened" size="mini"
                       @click="handleExportNoPage">导出全部订单
            </el-button>
          </el-col>
        </template>
      </right-toolbar>
    ...
    <virtual-scroll
      ...
      >
        ...
        <!-- 23. 收到条附件 -->
        <el-table-column v-if="columns[22].visible" show-overflow-tooltip label="收到条附件" align="center"
                         prop="receiveProof">
          <template #default="scope">
            <ExpandCursor>
              <div v-if="Array.isArray(scope.row.attachmentList)">
          ...
          <template slot-scope="scope">
            <div>
              <el-button size="mini" type="text" :disabled="scope.row.isAdjusted !== 1" v-if="!isAdjustOrder"
                         @click="handleCheckAdjust(scope.row)">查看调整单
              </el-button>
              <el-button size="mini" type="text" :disabled="scope.row.isAdjusted === 1"
                         @click="handleOrderItemInfo(scope.row)">调整单
              </el-button>
              <el-button v-if="isAdjustOrder" size="mini" type="text" @click="handleCheckPrevious(scope.row)">
                查看原单据
              </el-button>
    ...
    <div v-if="isAdjustOrder">
      <el-row>
        <el-card class="box-card">
          ...
          <el-alert title="订单列表点击调整单后，会在此生成调整单，不能对负数单进行调整，且负数单不可修改!"
                    type="warning" style="margin-bottom: 10px" show-icon effect="light"/>
          ...
          <div class="order-info-content">
            ...
            <div class="order-info-item">
              ...
              <span class="info-value total-amount">¥{{
                  Number(currentOrderInfo.allPayments || 0).toLocaleString()
                }}</span>
            </div>
          </div>
        </el-card>
          ...
          <div v-for="group in supplierInvoiceGroups" :key="group.companyId" class="supplier-group-card">
            ...
            <div class="supplier-group-header">
              <div class="supplier-info">
                ...
                <el-tag type="warning" size="small" style="margin-left: 10px">{{ group.invoices.length }} 条记录
                </el-tag>
              </div>
