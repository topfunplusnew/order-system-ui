...
import {formatValue} from "../api/tool/con";
...
<template>
  <div>
    <el-descriptions title="订单信息" size="mini" border>
      <el-descriptions-item label="日期">{{ formatValue(orderInfo.orderDate) }}</el-descriptions-item>
      ...
      <el-descriptions-item label="客户是否开票"
                            v-if="orderInfo.customerIsInvoice !== null && orderInfo.customerIsInvoice!== undefined && orderInfo.customerIsInvoice !== '' ">
        {{ orderInfo.customerIsInvoice === 0 ? '未开票' : '开票' }}
      </el-descriptions-item>
      <el-descriptions-item label="供应商是否开票"
                            v-if="orderInfo.isSupplierInvoice !== null && orderInfo.isSupplierInvoice!== undefined && orderInfo.isSupplierInvoice!== ''">
        {{ orderInfo.isSupplierInvoice === 0 ? '未开票' : '开票' }}
      </el-descriptions-item>
      <el-descriptions-item label="销售经理">{{ formatValue(orderInfo.saleManager) }}</el-descriptions-item>
      <el-descriptions-item label="车队">{{ formatValue(orderInfo.fleet) }}</el-descriptions-item>
      <el-descriptions-item label="陆运车牌">{{ formatValue(orderInfo.landCarNo) }}</el-descriptions-item>
      <el-descriptions-item label="陆运司机电话">{{ formatValue(orderInfo.landDriverTel) }}</el-descriptions-item>
      ...
      <el-descriptions-item label="海运费">{{ formatValue(orderInfo.seaFreight) }}</el-descriptions-item>
      <el-descriptions-item label="总货款">{{ formatValue(orderInfo.allPayments) }}</el-descriptions-item>
    </el-descriptions>
