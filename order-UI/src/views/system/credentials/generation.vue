...
<template>
  <div>
    ...
    <div style="margin: 19px 19px">
      ...
      <!--      选择单据类型后跳出的订单选择框-->
      <InfoDialog :title="'凭证生成'" :visible.sync="orderDialogVisible"
                  @update:visible="orderDialogVisible = false;">
        <!--        如果是订单-->
        <template #info>
          <!--    如果选择的是订单列表-->
          <OrderList v-if="DOC_TYPE===DocumentNumber.GOODS_ORDER"
          ...
          <!--        如果是发票买入  -->
          <InvoiceList v-if="DOC_TYPE===DocumentNumber.INVOICE_IN" :invoice-type="TableName.INVOICE_IN"
                       @update:selectedGoodsOrderList="handleEmitGoodsOrderList">
            <template #option>
              <!-- 生成订单中的运费 总货款 出厂货款的凭证-->
              <el-row>
          ...
          <!--        如果是发票卖出 那么就传递卖出列表-->
          <InvoiceList v-if="DOC_TYPE===DocumentNumber.INVOICE_OUT" :invoice-type="TableName.INVOICE_OUT"
                       @update:selectedGoodsOrderList="handleEmitGoodsOrderList">
            <template #option>
              <!-- 生成订单中的运费 总货款 出厂货款的凭证-->
              <el-row>
          ...
          <!--        第三方-->
          <InvoiceList v-if="DOC_TYPE===DocumentNumber.INVOICE_OTHER" :invoice-type="TableName.INVOICE_OTHER"
                       @update:selectedGoodsOrderList="handleEmitGoodsOrderList">
            <template #option>
              <!-- 生成订单中的运费 总货款 出厂货款的凭证-->
              <el-row>
      ...
      <!--       确认选择订单页面-->
      <InfoDialog :title="'确认选择信息'" :visible.sync="CheckDialogVisible"
                  @update:visible="CheckDialogVisible = false">
        <template #info>
          <!-- 订单确认生成票据-->
          <el-row v-if="DOC_TYPE === DocumentNumber.GOODS_ORDER">
            <!-- 订单的列表-->
            <el-table v-horizontal-scroll="'always'" fit border :data="selectedNeedOrderList"
          ...
          <el-row v-if="DOC_TYPE === DocumentNumber.INVENTORY">
            <el-table v-horizontal-scroll="'always'" border :data="selectedNeedOrderList"
