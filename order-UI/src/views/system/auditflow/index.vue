<template>
	<div class="app-container">
		...
		>
			<el-row>
				<el-steps :active="stepLength" align-center direction="vertical">
					<el-step v-for="(item, index) in checkStepList" :key="index">
						...
						<template #title>
							<el-button
								v-if="item.auditauthority.length === 0"
								icon="el-icon-circle-plus-outline"
								type="success"
								@click="selectAuditFlowPersons(index)"
								>选择审核人员
							</el-button>
							<el-button
								v-else
								...
								type="success"
								@click="selectAuditFlowPersons(index)"
								>已选择用户编号为{{ item.auditauthority }}的用户
							</el-button>
						</template>
						<template #description>
							<el-input
								v-model="item.flowname"
								type="text"
								placeholder="请输入审核名称"
							></el-input>
						</template>
			...
			<div slot="footer" class="dialog-footer">
				<el-button
					icon="el-icon-circle-plus-outline"
					type="success"
					@click="addCheckStateStep"
					>添加审核步骤</el-button
				>
				<el-button
					v-if="checkStepList.length !== 0"
					type="danger"
					@click="deleteCheckStateStep"
					>删除审核步骤
				</el-button>
		...
		<auditFlowInfo
			ref="auflowAddForm"
			:visible.sync="auditFlowPersonsVisible"
			@changeSelectedList="changeSelectListAudit"
			@deleteSelectedList="deleteSelectList"
		/>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		changeSelectListAudit(id) {
			if (typeof id === 'object') {
				...
				this.checkStepList[this.currentId].auditauthority = [
					...new Set(this.checkStepList[this.currentId].auditauthority)
				];
		...
		deleteSelectList(id) {
			if (typeof id === 'object') {
				this.checkStepList[this.currentId].auditauthority = this.checkStepList[
					this.currentId
				].auditauthority.filter(item => {
