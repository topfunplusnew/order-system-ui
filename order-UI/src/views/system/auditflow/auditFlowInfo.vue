    ...
    <el-dialog :close-on-click-modal="false" :show-close="false" v-bind="$attrs" title="选择审核人员" v-on="$listeners" @open="onOpen">
      <el-row>
        ...
        <el-col :span="18">
          ...
          <el-button v-if="ids.length !== 0" type="success" size="mini" @click="handleSelectAll">确认已选择的
          </el-button>
          <el-button v-if="ids.length !== 0" type="danger" size="mini" @click="handleDeleteAll">删除已选择的
          </el-button>
          <el-table v-loading="loading" border :data="userList" size="mini" :cell-style="()=>{return {padding:'2px'}}"
            ...
            <el-table-column key="nickName" label="用户昵称" align="center" prop="nickName"
                             :show-overflow-tooltip="true" />
            <el-table-column key="trueName" label="真实姓名" align="center" prop="trueName"
                             :show-overflow-tooltip="true" />
