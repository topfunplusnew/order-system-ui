<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tripReimbursementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="报销项" align="center" prop="item" />
      <el-table-column label="费用" align="center" prop="itemCost" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTripReimbursement(this.queryParams).then(response => {
        this.tripReimbursementList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTripReimbursement(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTripReimbursement(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTripReimbursement(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除出差报销编号为"' + ids + '"的数据项？').then(function() {
        return delTripReimbursement(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/tripReimbursement/export', {
        ...this.queryParams
