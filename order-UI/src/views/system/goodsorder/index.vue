...
<template>
  <div class="app-container">
    ...
    <el-table fit border v-loading="loading" :data="goodsOrderList" @selection-change="handleSelectionChange"
      id="printBox" v-horizontal-scroll="'always'" @header-dragend="changeColWidth" virtual-scroll max-height="750"
      ...
      <el-table-column show-overflow-tooltip label="陆运车牌" align="center" prop="landCarNo" v-show="columns[3].visible" />
      ...
      <el-table-column show-overflow-tooltip label="审核状态" align="center" prop="checkState" v-show="columns[13].visible"
        width="120">
        <template #default="scope">
    ...
    <UploadPath :before-upload="beforeUpload" :file-list="fileList" :handle-upload-visible="handleUploadVisible"
      :headers="headers" :submit-upload-all-files="() => submitUploadAllFiles('path')" :upload-file-url="uploadFileUrl"
    ...
    <el-dialog :close-on-click-modal="false" :show-close="false" title="陆运费申请" :visible.sync="landFreeDialogVisible"
      width="600px">
      <keep-alive>
    ...
    <Invoice :check-rules="CheckRules" :handle-commit-back-company="handleCommitBackCompany"
      ...
      :invoiceupdate-order-item-visible-visible="invoiceupdateOrderItemVisibleVisible" :list-company="listCompany"
      :query-company-name="queryCompanyName" :submitupdate-order-item-visible-title="submitupdateOrderItemVisibleTitle"
    ...
    <PrimativeOrderInfo :current-order-item-info="currentOrderItemInfo"
      :current-order-item-info-visible="currentOrderItemInfoVisible" @close="closePrimativeOrderInfo" />
  </div>
</template>
