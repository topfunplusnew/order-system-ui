      ...
      <el-row>
        ...
        <el-row>
          ...
          <el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" border
                    :data="statementList" lazy height="570px"
                    size="mini"
            ...
            <el-table-column align="center">
              ...
              />
              <el-table-column v-if="columns[1].visible" label="业务员" align="driverName" prop="salesman"
                               width="200" />
              <el-table-column v-if="columns[2].visible" label="区域" align="center" prop="region"
                               width="200" />
            </el-table-column>
            <!--            联系方式-->
            ...
            <!--            年份信息 遍历年份数组-->
            <el-table-column v-for="(item,index) in handleData(statementList,0)" v-if="statementList.length !==0"
                             :key="index" align="center" :label="item+`年`">
              <!--              遍历月份 先拿到该年份下的月份数据 然后在下面进行遍历-->
              <el-table-column v-for="(element,cols) in handleData(statementList,item)" :key="cols" label="发货量"
