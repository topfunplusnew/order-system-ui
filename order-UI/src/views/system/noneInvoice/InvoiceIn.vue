<template>
	<div class="app-container">
					...
					<el-row>
						...
						<el-col :span="12">
							<el-button
								size="mini"
								type="text"
								:disabled="scope.row.checkState !== '未申请'"
		...
		>
			<ApplyPayment
				:table-name="TableName.INVOICE_IN"
				:t-i-d="tID"
...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				companyType: [
					{ required: true, message: '请选择对方公司类型', trigger: 'blur' }
				],
				companyName: [
					{ required: true, message: '请输入对方公司名称', trigger: 'blur' }
				],
				invoiceCompanyName: [
	...
	watch: {
		...
		form: {
			handler() {
				this.invoiceAmount = Number(
					this.form.invoiceAmount * this.form.ticketPoint
				).toFixed(3);
			},
			deep: true
		}
	...
	created() {
		this.getList();
		if (
			localStorage.getItem('invoicein-columns') === 'null' ||
