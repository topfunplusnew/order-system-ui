...
<template>
  <div class="app-container">
    ...
    <el-table id="printBox" v-loading="loading" v-horizontal-scroll="'always'" border
              :data="lendMoneyList" size="mini" :cell-style="()=>{return {padding:'1px'}}" @selection-change="handleSelectionChange">
      <el-table-column v-if="columns[0].visible" label="押金公司" align="center" prop="futuresMarginCompany"
      ...
      <el-table-column v-if="columns[9].visible" label="我方开户行" align="center" prop="selfBankName" width="110"
                       show-overflow-tooltip />
      <el-table-column v-if="columns[10].visible" label="支付押金时间" align="center" prop="futuresDate"
                       show-overflow-tooltip
                       width="110" />
    ...
    <!-- 添加或修改向外部借出款信息对话框 -->
    <el-dialog :close-on-click-modal="false" :show-close="false" title="添加押金" :visible.sync="open"
          ...
          <el-col :span="12">
            ...
            <el-form-item label="对方账户" prop="targetAcountsName">
              <el-row>
                ...
                <el-col :span="3">
                  <SearchOption :get-data="listBankAccount" icon="el-icon-search" :limit-info="{acountsType:form.targetType==='其他'||form.targetType==='员工'?'':form.targetType}"
                                query-label="户名查找"
                                query-info="acountsName" :query-name="queryBank" @commitBack="handleCommitBack"
    ...
    <!--    回收弹窗-->
    <el-dialog :close-on-click-modal="false" :show-close="false" title="收回资金操作"
               :visible.sync="giveRecoverMoneyShow" width="40%" append-to-body>
      <el-row>
